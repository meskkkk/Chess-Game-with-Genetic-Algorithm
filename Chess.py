...
class GeneticAlgorithm:
    ...
    def genetic_algorithm(
        ...
        for generation in range(generations):
            ...
            fitness_scores = {
                tuple(sequence): self.evaluate_sequence(sequence)
                for sequence in population
            }

            # Step 3: Select the top sequences based on fitness (elitism)
            sorted_population = sorted(
                population, key=lambda seq: fitness_scores[tuple(seq)], reverse=True
            )
            top_sequences = sorted_population[
                : population_size // 2
            ]  # Keep the top 50% of sequences

            # Step 4: Crossover - Create new sequences by combining top sequences
            children = self.crossover_sequences(
                top_sequences, population_size - len(top_sequences)
            )
...
class ChessGUI:
    def __init__(self):
        ...
        self.piece_images = {
            piece: ctk.CTkImage(
                light_image=Image.open(f"images/{piece}.png"), size=(70, 70)
            )
            for piece in [
                "white_knight",
                "white_pawn",
    ...
    def menu(self):
        ...
        self.bg_photo = ImageTk.PhotoImage(Image.open("images/Chess Wallpaper.jpeg"))
        canvas.create_image(0, 0, image=self.bg_photo, anchor="nw")

        # Overlay transparent image
        self.overlay_photo = ImageTk.PhotoImage(Image.open("images/chess.png"))
        ...
        ctk.CTkButton(
            menu_frame,
            text="Play",
            command=lambda: [self.board_gui(), menu_frame.destroy()],
            **button_style,
        ).place(relx=0.5, rely=0.5, anchor="center")
        ctk.CTkButton(
            menu_frame, text="Quit", command=self.root.destroy, **button_style
        ...
        for i in range(8):
            for j in range(8):
                ...
                b = ctk.CTkButton(
                    ...
                    border_width=1,
                    corner_radius=0,
                    image=self.piece_images.get(self.get_piece(i, j)),
    ...
    def get_piece(self, row, col):
        ...
        )  # python-chess uses a different coordinate system
        if piece is None:
            return "default"
        piece_map = {
            chess.PAWN: "white_pawn" if piece.color == chess.WHITE else "black_pawn",
            chess.KNIGHT: (
                "white_knight" if piece.color == chess.WHITE else "black_knight"
            ),
            chess.BISHOP: (
                "white_bishop" if piece.color == chess.WHITE else "black_bishop"
            ),
    ...
    def highlight_moves(self, row, col):
        ...
        for move in self.board.legal_moves:
            if move.from_square == piece_square:
                ...
                if (
                    destination_piece
                    and destination_piece.color
                    != self.board.piece_at(piece_square).color
                ):
            ...
            for j in range(8):
                self.squares[i][j].configure(
                    image=self.piece_images.get(
                        self.get_piece(i, j), self.piece_images["default"]
                    )
                )
    ...
    def make_black_move(self):
        ...
        if self.turn == chess.BLACK:
            best_move = self.genetic_algorithm.genetic_algorithm(
                self.board
            )  # Get the best move using GA
